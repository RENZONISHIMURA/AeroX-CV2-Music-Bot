...
     context: 'assets',
    },
    // New path to check the current directory (where the class file is)
    { path: process.cwd(), context: 'current_dir' },
   ];
...
    try {
     // Register WorkSans.ttf (assuming it contains all weights or we only need a single registration)
     // Note: If you have separate files like WorkSans-Bold.ttf, you'll need multiple registerFromPath calls.
     // For simplicity, assuming 'WorkSans.ttf' for all.
     GlobalFonts.registerFromPath(
      path.join(fontPath.path, 'WorkSans.ttf'),
     	'Work Sans',
     );
     
     // You should remove or comment out the old font registrations if you are replacing them
     // For now, I'll remove the Inter and Noto Sans registrations to switch completely.
     // If WorkSans.ttf does not include Bold/SemiBold, you may need to register specific files:
     /*
     GlobalFonts.registerFromPath(path.join(fontPath.path, 'WorkSans-Bold.ttf'), 'Work Sans Bold');
     GlobalFonts.registerFromPath(path.join(fontPath.path, 'WorkSans-SemiBold.ttf'), 'Work Sans SemiBold');
     GlobalFonts.registerFromPath(path.join(fontPath.path, 'WorkSans-Medium.ttf'), 'Work Sans Medium');
     */


     console.log(`[MusicCard] Font 'Work Sans' registered successfully from: ${fontPath.path} (${fontPath.context})`);
     fontsRegistered = true;
     break; // Stop after the first successful registration path
    } catch (e) {
...
  // **UPDATED: Use the Work Sans font family name**
...
   truncated.length > 0
  ) {
   truncated = truncated.slice(0, 0); // Corrected: was `truncated.slice(0, -1);`
  }
...
   title,
   contentWidth - 20,
   '38px "Work Sans"', // Changed from "Inter Bold"
  );

  ctx.save();
  // **UPDATED: Use Work Sans**
  ctx.font = '38px "Work Sans"'; // Changed from "Inter Bold"
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  
...
   artist,
   contentWidth - 20,
   '26px "Work Sans"', // Changed from "Inter Medium"
  );

  ctx.save();
  // **UPDATED: Use Work Sans**
  ctx.font = '26px "Work Sans"'; // Changed from "Inter Medium"
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.shadowColor = 'rgba(0, 0, 0, 0.8)';
  ctx.shadowBlur = 12;
...
  ctx.fillStyle = '#a0b0c0';
  // **UPDATED: Use Work Sans**
  ctx.font = '18px "Work Sans"'; // Changed from "Inter Medium"
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
...
  ctx.fillStyle = '#e0e8f0';
  // **UPDATED: Use Work Sans**
  ctx.font = '16px "Work Sans"'; // Changed from "Inter SemiBold"
